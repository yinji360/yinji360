	  ...
	  @foreach ($changelist as $value)
		<div class="teachinfor">
             <ul class="teachlist clearfix">
                ...
                <li>
					<span class="fr wid"><a href="javascript:void(0);" onclick="reviceClass({{$value->id}},2)">拒绝</a></span>				
					<span class="fr wid"><a href="javascript:void(0);" onclick="reviceClass({{$value->id}},1)">接受</a></span>
                   <span class="fl">{{$value->name}}（手机：{{$value->tel}}）向您转让{{$value->class_name}}</span>
                </li>
...
<script type="text/javascript">
	...
	function reviceClass(id,type){
		var column_id = $("#column_id").val();
		$.ajax( {  
			type : "get",  
			url : "/classes/receiveclass", 
			data : "id="+id+"&type="+type+'&column_id='+column_id,
			dataType:"json",  
