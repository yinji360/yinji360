...
class ClassmateController extends BaseController {
    ...
    {
        $query = Input::only('ids');
		$mate_ids = substr($query['ids'],1);
		$classmate_ids = explode(',',$mate_ids);
	...
	public function postUpdate()
    {
        ...
        foreach ($classmate_ids as $key => $classmate_id) {
			$classmate = Classmate::find($classmate_id);
			$classmate->class_id = $query['class_id'];
			$classmate->save();
			//$m->editInfoById($classmate_id,array('class_id'=>$query->class_id));
